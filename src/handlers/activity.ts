...
function countReactions(message: Record<string, any>): number {
  ...
  return message.reactions.reduce((sum: number, reaction: { count?: number }) => sum + (reaction.count || 0), 0);
}
...
export async function getUserChannelActivityHandler(args: unknown) {
  ...
  for (const channel of channelsToProcess) {
    try {
      ...
      if (!historyResponse.ok) {
        console.error(`Failed to get history for channel ${channel.id}: ${historyResponse.error}`);
        continue;
      }
      ...
      processedMessages.sort((a, b) => {
        ...
        const importanceA = (a.reaction_count || 0) * 1.0 + 
                         (a.reply_count || 0) * 1.5 + 
                         (a.has_mention ? 2 : 0) + 
                         timeBonusA;
        const importanceB = (b.reaction_count || 0) * 1.0 + 
                         (b.reply_count || 0) * 1.5 + 
                         (b.has_mention ? 2 : 0) + 
                         timeBonusB;
  ...
  const filteredSummaries = channelSummaries.filter(summary => summary.messages.length > 0);
  
  // 重要度でソート（メッセージの合計重要度）
  filteredSummaries.sort((a, b) => {
    const importanceA = a.messages.reduce((sum, msg) => sum + (msg.reaction_count || 0) * 1.0 + (msg.reply_count || 0) * 1.5 + (msg.has_mention ? 2 : 0) + (msg.reaction_count || 0) * 0.8 + (msg.reply_count || 0) * 1.2 + (msg.has_mention ? 2 : 0) * 0.8 + 1, 0) / Math.sqrt(a.messages.length);
    const importanceB = b.messages.reduce((sum, msg) => sum + (msg.reaction_count || 0) * 1.0 + (msg.reply_count || 0) * 1.5 + (msg.has_mention ? 2 : 0) + (msg.reaction_count || 0) * 0.8 + (msg.reply_count || 0) * 1.2 + (msg.has_mention ? 2 : 0) * 0.8 + 1, 0) / Math.sqrt(b.messages.length);
    return importanceB - importanceA;
  });
