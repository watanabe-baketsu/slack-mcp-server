...
export async function listChannelCanvasesHandler(args: unknown) {
  ...
  try {
    ...
    try {
      console.log(`Trying conversations.canvases.list for channel ${parsedArgs.channel_id}`);
    ...
    console.log(`Files found in channel ${parsedArgs.channel_id}:`, 
    ...
    // 方法3: 直接API呼び出しを試みる
    try {
      console.log(`Trying direct API call for files.list in channel ${parsedArgs.channel_id}`);
